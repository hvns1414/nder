...
def fancy_intro():
    colorama.init()
    print(Fore.CYAN + "=" * 50)
    print(Fore.YELLOW + pyfiglet.figlet_format("nder"))
    print(Fore.CYAN + "=" * 50 + Style.RESET_ALL)
    print(Fore.GREEN + "\nWelcome to nder!" + Style.RESET_ALL)
    time.sleep(1)
    print(Fore.MAGENTA + "Fetching cryptocurrency data..." + Style.RESET_ALL)
    time.sleep(1)
...
def plot_stock_chart(data, ticker):
    ...
    ax.set_title(f"Crypto Prediction for {ticker}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Price")
    ax.legend()
    plt.savefig("crypto_chart.png", dpi=300)
    plt.close()

def main():
    fancy_intro()
    ticker = input("Enter cryptocurrency ticker (e.g., BTC-USD): ")
    data = fetch_stock_data(ticker)
    data = generate_signals(data)
    plot_stock_chart(data, ticker)
    print("Crypto chart saved as 'crypto_chart.png'")
